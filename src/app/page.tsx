...
import { Info, AlertCircle, Calculator, CheckCircle } from 'lucide-react';
        ...
        <main className="flex min-h-screen flex-col items-center justify-start p-4 sm:p-6 md:p-10 lg:p-12 bg-gray-100 dark:bg-gray-950">
            <Card className="w-full max-w-xl shadow-xl border dark:border-gray-700 bg-white dark:bg-gray-900">
                <CardHeader className="bg-gray-50 dark:bg-gray-800/50 p-5 rounded-t-lg border-b dark:border-gray-700">
                    <CardTitle className="text-xl sm:text-2xl font-bold text-gray-800 dark:text-gray-100">Calculadora Óptica RPI</CardTitle>
                    <CardDescription className="text-gray-600 dark:text-gray-400">Calcule la Potencia Intermedia (PI).</CardDescription>
                </CardHeader>
                ...
                <CardContent className="p-5 sm:p-6 space-y-6">
                    ...
                    <section className="space-y-5 p-4 border dark:border-gray-700/50 rounded-lg bg-white dark:bg-gray-800/20 shadow-sm">
                        <h3 className="text-base sm:text-lg font-semibold border-b dark:border-gray-600 pb-2 mb-4 text-gray-700 dark:text-gray-300">1. Visión Lejana (VL)</h3>
                        <PrescriptionInputGroup
                            ...
                            // containerClass no necesita highlight aquí
                        />
                         <div className="pt-4">
                             <PrescriptionInputGroup
                    ...
                    <section className="space-y-4 p-4 border dark:border-gray-700/50 rounded-lg bg-white dark:bg-gray-800/20 shadow-sm">
                        <h3 className="text-base sm:text-lg font-semibold border-b dark:border-gray-600 pb-2 mb-4 text-gray-700 dark:text-gray-300">2. Adición (ADD)</h3>
                        <div className="space-y-4">
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-4 items-start">
                                <div className="space-y-1.5">
                                    ...
                                    <Input id="odAdd" type="text" inputMode="decimal" placeholder="+2.00" value={odAdd}
                                        ...
                                        className={cn( // Usar cn para combinar clases
                                            'transition-colors placeholder:text-muted-foreground/60 h-9', // Clases base del input
                                            highlightOdAdd ? C.INPUT_HIGHLIGHT_CLASS : '', // Highlight condicional
                                            getErrorClass("odAdd", fieldErrors) // Clase de borde normal o de error
                                        )}
                                ...
                                <div className="space-y-1.5">
                                    ...
                                    <Input id="oiAdd" type="text" inputMode="decimal" placeholder="+2.00" value={oiAdd}
                                           ...
                                           className={cn( // Usar cn para combinar clases
                                            'transition-colors placeholder:text-muted-foreground/60 h-9', // Clases base
                                            highlightOiAdd ? C.INPUT_HIGHLIGHT_CLASS : '', // Highlight
                                            getErrorClass("oiAdd", fieldErrors) // Error
                                        )}
                            ...
                            <div className="pt-1 text-center sm:text-right">
                                <Button type="button" variant="outline" size="sm" onClick={handleCalculateAdd} className="shadow-sm">
                                    <Calculator className="mr-2 h-4 w-4" /> Calcular ADD desde VC/VL
                                </Button>
                            </div>
                        </div>
                    </section>
                    ...
                    <section className={`space-y-5 p-4 border dark:border-gray-700/50 rounded-lg bg-white dark:bg-gray-800/20 shadow-sm transition-colors`}>
                        <h3 className="text-base sm:text-lg font-semibold border-b dark:border-gray-600 pb-2 mb-4 text-gray-700 dark:text-gray-300">3. Visión Cercana (VC)</h3>
                         <PrescriptionInputGroup
                            eyeLabel="Ojo Derecho (OD)"
                            eyePrefix="od"
                            sectionPrefix="Vc"
                            ...
                            handleStepChange={handleStepChange}
                            handleInputChange={handleInputChange}
                            containerClass={highlightOdVc ? C.HIGHLIGHT_CONTAINER_CLASS : undefined} // Pasar clase de highlight o undefined (usará default)
                        />
                         <div className="pt-4">
                              <PrescriptionInputGroup
                                eyeLabel="Ojo Izquierdo (OI)"
                                ...
                                handleStepChange={handleStepChange}
                                handleInputChange={handleInputChange}
                                containerClass={highlightOiVc ? C.HIGHLIGHT_CONTAINER_CLASS : undefined} // Pasar clase de highlight o undefined
                            />
                         </div>
                    ...
                    {(resultOD_PI_075 !== null || resultOI_PI_075 !== null || resultOD_PI_125 !== null || resultOI_PI_125 !== null) && errorMessages.length === 0 ? (
                        <section className="space-y-4 p-4 mt-5 border-2 rounded-lg bg-green-100 dark:bg-green-900/30 border-green-500 dark:border-green-600 shadow-md animate-fade-in">
                            <h3 className="text-base sm:text-lg font-semibold border-b border-green-400 dark:border-green-600 pb-2 mb-4 text-green-800 dark:text-green-300 flex items-center gap-2">
                                <CheckCircle className="h-5 w-5" /> Resultados: Potencia Intermedia (PI)
                            </h3>
